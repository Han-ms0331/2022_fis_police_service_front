        ...
        <div style={{width: "100%", height: "530px", padding: "10px"}}>
            <div style={{marginBottom: "20px"}}>
                <InputContainer labelContent="인/아웃바운드: " inputName="in_out" inputType="select" contents={["인", "아웃"]}
                                width="100px" row="1" defaultValue={data === undefined ? "" : data.in_out} setValueFunction={handleInputFormChange}/>
            </div>
            <div style={{marginBottom: "20px"}}>
            ...
            <div style={{marginBottom: "20px"}}>
                <InputContainer labelContent="시설 참여여부: " inputName="participation" inputType="select"
                                ...
                                defaultValue={data === undefined ? "" : data.participation} setValueFunction={handleInputFormChange}/>
            </div>
            <div style={{marginBottom: "20px"}}>
                <InputContainer labelContent="담당자 이름: " inputName="c_manager" inputType="text" width="150px" row="1"
                                defaultValue={data === undefined ? "" : data.c_manager} setValueFunction={handleInputFormChange}/>
            </div>
            <div style={{marginBottom: "20px", display: "flex", alignItems: "center"}}>
                <InputContainer labelContent="담당자 이메일: " inputName="m_ph" inputType="text" width="200px" row="1"
                                defaultValue={data === undefined ? "" : mail[0]} setValueFunction={handleInputFormChange}/>
                <div style={{margin: "0px 5px"}}>@</div>
                <CustomInput inputName={"email"} type={"select"}
                             contents={["naver.com", "google.com", "hanmail.com", "직접입력"]} width={"150px"} row={"1"} defaultValue={mail[1]} setValueFunction={handleInputFormChange}/>
            </div>
            <div style={{marginBottom: "20px"}}>
                <InputContainer labelContent="담당자 전화번호: " inputName="m_email" inputType="text" width="150px" row="1"
                                defaultValue={data === undefined ? "" : data.m_ph} setValueFunction={handleInputFormChange}/>
            </div>
            <div style={{marginBottom: "20px"}}>
                <InputContainer labelContent="시설 특이사항: " inputName="center_etc" inputType="text" width="100%" rows="3"
                                defaultValue={data === undefined ? "" : data.center_etc} setValueFunction={handleInputFormChange}/>
            </div>
            <div style={{marginBottom: "20px"}}>
                <InputContainer labelContent="현장요원 특이사항: " inputName="agent_etc" inputType="text" width="100%" rows="3"
                                defaultValue={data === undefined ? "" : data.agent_etc} setValueFunction={handleInputFormChange}/>
            </div>
